...
应用顶部的“外观”菜单可在三套皮肤间切换：晨雾是淡蓝灰浅色，深海适合夜间，暖砂是米白暖棕。选择会保存在本机，下次启动继续使用；切换外观不会中断播放或下载。
...
- **磁力链接**：点击“粘贴磁力”，输入完整的 `magnet:?` 链接。SeedStream 会通过 DHT、链接自带 Tracker 和内置备用 Tracker 寻找元数据节点；解析时可随时点击“取消解析”、右上角关闭或按 Esc，不需要等待超时。

两种方式都只建立任务和展示清单，不会自动播放或永久下载。磁力元数据与实际传输速度取决于可用节点；节点为零时，软件无法从网络取得内容。取消解析会终止临时任务并清理本次元数据缓存。
...
`pnpm smoke` 使用本机生成的合法测试数据，建立仅限回环地址的 WebTorrent 客户端，验证真实磁力元数据交换和字节范围播放，并启动一次隔离用户目录的隐藏 Electron 窗口。它不会访问公共种子或第三方网站。
