...
SeedStream 是一个面向 macOS 和 Windows 的本地桌面种子播放器与下载器。它支持本地 `.torrent` 和磁力链接；导入后可先查看文件清单，再选择边下边播或永久下载。
...
- **Torrent 文件**：点击“打开 `.torrent`”，或把本机 `.torrent` 文件拖入左侧“加入种子”区域。
- **磁力链接**：点击“粘贴磁力”，输入完整的 `magnet:?` 链接。SeedStream 会寻找节点并取得文件清单，成功后作为普通任务加入。

两种方式都只建立任务和展示清单，不会自动播放或永久下载。磁力元数据与实际传输速度取决于可用节点；节点为零时，软件无法从网络取得内容。
...
`pnpm smoke` 使用本机生成的合法测试数据，建立两个仅限回环地址的 WebTorrent 客户端，验证字节范围播放，并启动一次隔离用户目录的隐藏 Electron 窗口。它不会访问公共种子或第三方网站。
...
- 主界面不加载远程网页，也不内置资源搜索或第三方搜索源连接
- 磁力链接在主进程中校验协议、长度和 info hash 后才交给下载引擎
